...
def thread(params):
    ...
    # Track progress
    done_set = set(
        Path(f"./progress/progress{args.dataset}seq.txt").read_text().splitlines()
    )
    ...
    if args.pad and len(occupied_slices) % sl > int(sl / 2):
        ...
        while len(occupied_slices) < goal:
            if right_index < len(vol):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Length of volumes sequences to be generated",
    )
