...
def parse_args():
    ...
    parser.add_argument(
        "--perplexity", type=int, default=15, help="Perplexity parameter for t-SNE"
    )
    parser.add_argument(
        "--random_state", type=int, default=42, help="Random state for reproducibility"
    )
    parser.add_argument(
        "--interactive", action="store_true", help="Generate interactive HTML plots"
    )
...
def reshape_features(features, layer_name):
    ...
    print(
        f"Reshaped {layer_name} features from {original_shape} to {flattened_features.shape}"
    )
...
def visualize_features(features_dict, labels, modality_masks, args):
    ...
    for layer_name in target_layers:
        ...
        pca_features, explained_variance, component_variances = apply_pca(
            reshaped_features, n_components=3, random_state=args.random_state
        )
        ...
        if args.interactive:
            interactive_pca_path = os.path.join(
                args.output_dir, f"{layer_name}_pca_3d_interactive.html"
            )
        ...
        if args.interactive:
            interactive_tsne_path = os.path.join(
                args.output_dir, f"{layer_name}_tsne_3d_interactive.html"
            )
            create_interactive_3d_plot(
                tsne_features,
                labels,
