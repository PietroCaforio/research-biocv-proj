...
def parse_args():
    parser = argparse.ArgumentParser(description="Test multimodal model and extract features")
    ...
    parser.add_argument("--output_layers", nargs='+', default=["classification"], 
...
def extract_features(model, data_loader, device, output_layers):
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader, desc="Extracting features"):
            # Get data from batch
            ct_volumes = batch["ct_volume"].float().unsqueeze(1).repeat(1, 3, 1, 1, 1).to(device)
            ...
            # Forward pass
            outputs = model(ct_volumes, wsi_volumes, modality_flag=modality_mask, output_layers=output_layers)
...
def save_features(features_dict, labels, modality_masks, patient_ids, output_dir):
    ...
    for layer_name, features in features_dict.items():
        ...
        print(f"Saved features from {layer_name} to {feature_file}, shape: {features.shape}")
    ...
    metadata = {
        ...
        "feature_shapes": {layer: features.shape for layer, features in features_dict.items()},
        "label_distribution": {int(label): int(np.sum(labels == label)) for label in np.unique(labels)}
    }
