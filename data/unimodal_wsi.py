...
class UnimodalWSIDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split:str,dataset_path:str = None, transform = None):
        ...
        #Generate a dictionary from the labels file where key is the patient_id and value is the actual label (G1, G2, G3)
            ...
            for row in split:
                ...
                for folder in glob.glob(f'{wsi_path}/*{row}*'):
                    ...
                    for file in os.listdir(folder):
                        # Add them to the items list as a dictionary with "slide_folder", "patient_id", "np.array as image" 
                        self.items.append({"patient_id": row, "slide_folder": folder, "patch": file})
                        #Update the classfrequency
                        self.classfreq[self.labels[row]] += 1 # the class frequency is given by the #patches per class                         
...
def sanity_check_dataset():
    ...
    dataset = UnimodalWSIDataset(split='all', dataset_path =  "data/processed/processed_CPTAC_PDA_71_3D", transform = None)
