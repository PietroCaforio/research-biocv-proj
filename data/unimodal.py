...
class UnimodalCTDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split: str, dataset_path: str = None, transform=None):
        ...
        labels_lines = (
            Path(f'{os.path.join(self.dataset_path,"labels.txt")}')
            .read_text()
            .splitlines()
        )

        self.labels = {
            k.strip(): v.strip() for k, v in (line.split(",") for line in labels_lines)
        }
        ...
        with open(f"{os.path.join(self.dataset_path,split)}.txt") as split:  # type: ignore
            for row in split:
                ...
                for file in os.listdir(os.path.join(self.dataset_path, "CT/" + row)):
                    ...
                    self.items.extend(
                        [row + "/" + file + "_" + str(i) for i in range(len(npy_ct))]
                    )
    ...
    def __getitem__(self, index):
        ...
        scan_frame = np.load(
            os.path.join(self.dataset_path, "CT/" + item.split("_")[0])
        )[int(item.split("_")[1])]
        ...
        return {"patient_id": patient_id, "frame": scan_frame, "label": item_class}
