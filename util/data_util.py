...
if os.name == "nt":
    OPENSLIDE_PATH = "..\\..\\Openslide\\openslide-bin-4.0.0.3-windows-x64\\bin"
    with os.add_dll_directory(OPENSLIDE_PATH):  # type: ignore
        import openslide
else:
    import openslide
...
def get_volume_clinicalTrialTimePoint(folder_path):
    """
    This function returns a dictionary with the metadata
    of the dicom volume in the folder given in input
    Args:
...
def load_single_volume(folder_path):
    """
    Returns a tuple with the volume of a single patient,
    its mean voxel dimension and a list of DICOM slices
    metadata (including z-coordinates).

    Args:
        folder_path (str): path to the folder containing the volume in dicom files
    Returns:
        a tuple ( the volume as np.array,
        the mean voxel dimension of the volume,
        list of DICOM slices metadata)
    """
    img_vol = []
    voxel_z = []
    voxel_x = []
    ...
    for path, _, files in sorted(os.walk(folder_path)):
        for filename in sorted(files):
            ...
            if filename.endswith(dcm_ext):
                ...
                # Append the DICOM slice to the list
                # (to access metadata like ImagePositionPatient)
...
def get_occupied_slices(rtstruct_path, dicom_slices, direction="axial"):
    ...
    if os.name == "nt":
        if rtstruct_path.startswith("\\\\"):
            rtstruct_path = "\\\\?\\UNC\\" + rtstruct_path[2:]
        else:
    ...
    if direction == "sagittal":
        slice_z_positions = [
            float(slice.ImagePositionPatient[0]) for slice in dicom_slices
        ]
    elif direction == "coronal":
        slice_z_positions = [
            float(slice.ImagePositionPatient[1]) for slice in dicom_slices
        ]
    else:
        slice_z_positions = [
            float(slice.ImagePositionPatient[2]) for slice in dicom_slices
        ]
    ...
    for contour_seq in contour_sequences:
        for contour in contour_seq.ContourSequence:
            ...
            if direction == "axial":
                contour_z_values = contour_data[
                    2::3
                ]  # Every third value is a z-coordinate
            elif direction == "coronal":
            ...
            for z in contour_z_values:
                ...
                for i, slice_z in enumerate(slice_z_positions):
                    ...
                    if math.isclose(z, slice_z, abs_tol=0.999):
                        ...
                        occupied_slices.add(
                            slice_index
                        )  # Add the index to the occupied_slices set
...
def remap_occupied_slices(occupied_slices, zoom_factor_z):
    """
    Remap and fill the indices of occupied slices from the original
    volume to the normalized volume.
...
def preprocess(volume: np.array, target_shape: list) -> np.array:
    ...
    Args:
        ...
        target_shape (list): list containing the target shape of the volume
...
def get_patients_ids(dataset_path):
    ...
    This function assumes that it's a unique folder with svs files
    named like: C3L-00189-21.svs
    Args:
        dataset_path (str)
    Returns:
...
def get_thumbnail(slide, level):
    """This function retrieves, through the openslide library,
    the image at the magnification level provided
    Args:
...
def apply_otsu_threshold(image):
    ...
    Returns:
        the binary mask (np.array): a boolean mask indicating the
        pixels where the gray level is under the threshold
    """
...
def extract_patches(
    image, binary_mask, patch_size=(224, 224), threshold=0.9, max_patches=16
):
    ...
    Args:
        image (np.array)
        binary_mask (np.array): the binary mask indicating where
        the tissue is present in the image
    ...
    for i in range(0, h - ph + 1, step):
        for j in range(0, w - pw + 1, step):
            if len(patches) >= max_patches:
                break
            patch = image[i : (i + ph), j : (j + pw)]  # noqa: E203
            mask_patch = binary_mask[i : (i + ph), j : (j + pw)]  # noqa: E203
            # print(mask_patch)
...
def calculate_coverage(
    slide, patches, level, level0_binary_mask, patch_size=(224, 224)
):
    """
    This function calculates the total tissue coverage of the
    patches onto the largest magnification level manageable.
    ...
    for _, _, i, j in patches:
        ...
        mask_patch_level0 = level0_binary_mask[
            i0 : i0 + ph0, j0 : j0 + pw0  # noqa: E203
        ]
...
def find_best_level(
    slide, patch_size=(224, 224), threshold=0.9, max_patches=16, level=None
):
    """
    This function finds the level of magnitude onto which to apply the patches in order to
    maximize the tissue area covered by the patches (this function is meant to tackle the
    problem of different scales of tissues in the wsi)
    ...
    # Iterate through every level and calculate the coverage of
    # the patches calculated and find the best one
    for level in range(1, slide.level_count):
        thumbnail = get_thumbnail(slide, level)
        binary_mask = apply_otsu_threshold(thumbnail)
        patches = extract_patches(
            thumbnail, binary_mask, patch_size, threshold, max_patches
        )

        if len(patches) >= max_patches:
            coverage = calculate_coverage(
                slide, patches, level, l0binary_mask, patch_size
            )
    ...
    if len(best_patches) < max_patches:
        raise ValueError(
            "Not enough patches with the required tissue content found at any level."
        )

    return best_level, best_patches


def get_patches_at_level(
    slide, patch_size=(224, 224), threshold=0.9, max_patches=16, level=1
):
    """
    This function returns "max_patches" patches of the wsi image taken in input at
    the fixed 'level' level of magnitude.
    ...
    Args:
        ...
        threshold (float): minimum percentage of tissue we want in each patch
        max_patches (int): number of patches we want to obtain
        level (int): level of magnitude of the OpenSlide wsi onto which we want
        to retrieve the patches
    Returns:
...
def save_patches_as_numpy(patches, output_path, patient_id):
    """
    This function saves the given patches into a directory, in case the directory
    doesn't exist, it creates it automatically.
    Args:
        patches (list of np.array)
