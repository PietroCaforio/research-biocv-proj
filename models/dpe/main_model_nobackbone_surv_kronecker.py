...
class fusion_layer(nn.Module):
    def __init__(self, d_model=64, dim_hider=32, nhead=8, dropout=0.1):
        super().__init__()
        # From Pathomic Fusion Mahmoodlab: https://github.com/mahmoodlab/PathomicFusion/blob/master/fusion.py#L154
        # Path
        self.linear_path = nn.Sequential(nn.Linear(d_model, dim_hider), nn.ReLU())
        # Rad
        self.linear_rad = nn.Sequential(nn.Linear(d_model, dim_hider), nn.ReLU())
        # Pos
        self.linear_pos = nn.Sequential(nn.Linear(d_model, dim_hider), nn.ReLU())
        # decoder:
        self.decoder = nn.Sequential(nn.Linear(dim_hider, d_model), nn.ReLU(), nn.Dropout(p=dropout))
        
    def forward(self, f1, f2, pos):
        
        # Reduce dimensionality for kronecker
        o1 = self.linear_rad(f1)
        o2 = self.linear_path(f2)
        pos = self.linear_pos(pos)
        
        
        o123 = o1 * o2 * pos
        
        o123 = self.decoder(o123)
        return o123
