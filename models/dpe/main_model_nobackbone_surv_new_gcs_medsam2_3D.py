...
class MADPENetNoBackbonesSurv(nn.Module):  # ModalityAwareDPENet da decidere nome
    ...
    def __init__(
        ...
        self.rad_adapter = nn.Sequential(
            # Now Conv3d over (256 × 32 × 32) volumes, with 66 input channels
            nn.Conv3d(
                in_channels=66,   # still 66
                out_channels=self.inter_dim,      # whatever intermediate dim you want
                kernel_size=3,
                stride=1,
                padding=1,                        # keep the same spatial dims
            ),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.BatchNorm3d(self.inter_dim),
            ...
            # Collapse the entire (D,H,W) volume down to 1×1×1 per channel
            nn.AdaptiveAvgPool3d(output_size=1),

            # This will give you a shape of [B, inter_dim, 1, 1, 1], so flatten to [B, inter_dim]
        ...
        self.token_adapt_rad = nn.Sequential(
            # 3D‐convolutional “adapter” over a [B, 66, 256, 32, 32] tensor:
            nn.Conv3d(
                in_channels=66,   # 66
                out_channels=self.inter_dim,
                kernel_size=3,
            ...
            # collapse each (256×32×32) volume to 1×1×1 per channel
            nn.AdaptiveAvgPool3d(output_size=1),

            # now shape is [B, inter_dim, 1, 1, 1] → flatten to [B, inter_dim]
            nn.Flatten(start_dim=1),

            # exactly the same MLP layers as before:
            nn.Linear(self.inter_dim, self.inter_dim),
            nn.GELU(),
            nn.Dropout(0.1),
