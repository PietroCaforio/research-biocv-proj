...
class RadAdapter2p5D(nn.Module):
    """
    2.5D adapter for radiology features.
    Input:  x of shape [B, 66, 256, 32, 32]
    Output: tensor of shape [B, inter_dim]
    """

    def __init__(self, 
                 slice_in_channels: int = 256, 
                 slice_emb_dim: int = 64, 
                 inter_dim: int = 128):
        super().__init__()
        self.slice_emb_dim = slice_emb_dim
        self.inter_dim = inter_dim

        # 1) Per‐slice 2D encoder: [B*S, 256, 32, 32] → [B*S, slice_emb_dim]
        self.slice2d = nn.Sequential(
            nn.Conv2d(slice_in_channels, slice_emb_dim, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.BatchNorm2d(slice_emb_dim),

            nn.Conv2d(slice_emb_dim, slice_emb_dim, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.BatchNorm2d(slice_emb_dim),

            nn.AdaptiveAvgPool2d((1, 1)),  # → [B*S, slice_emb_dim, 1, 1]
            nn.Flatten(),                  # → [B*S, slice_emb_dim]
        )

        # 2) 1D conv‐based fusion across the 66 slices:
        self.conv1 = nn.Conv1d(
            in_channels=slice_emb_dim,
        ...
        self.bn2   = nn.BatchNorm1d(inter_dim)

        # 3) Global pooling over the 66‐length dimension
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        returns: [B, inter_dim]
        """
        B, S, C, H, W = x.shape  # S = 66

        # → [B*S, 256, 32, 32]
        x = x.view(B * S, C, H, W)

        # per‐slice encode → [B*S, slice_emb_dim]
        x = self.slice2d(x)

        # → [B, S, slice_emb_dim]
        x = x.view(B, S, self.slice_emb_dim)

        # → [B, slice_emb_dim, S]
        x = x.permute(0, 2, 1)

        # 1D conv block #1: [B, slice_emb_dim, S] → [B, inter_dim, S]
        x = self.conv1(x)
        x = self.relu1(x)
        x = self.drop1(x)
        x = self.bn1(x)

        # 1D conv block #2: [B, inter_dim, S] → [B, inter_dim, S]
        x = self.conv2(x)
        x = self.relu2(x)
        x = self.drop2(x)
        x = self.bn2(x)

        # global‐pool over the 66 slices → [B, inter_dim, 1]
        x = self.pool(x)
...
class TokenAdaptRad2p5D(nn.Module):
    """
    2.5D token adapter for radiology features.
    Input:  x of shape [N, 66, 256, 32, 32]
    Output: tensor of shape [N, token_dim]
    """
    ...
    def __init__(self,
        ...
        # 1) Per‐slice 2D encoder: [N*S, 256, 32, 32] → [N*S, slice_emb_dim]
        self.slice2d = nn.Sequential(
            nn.Conv2d(slice_in_channels, slice_emb_dim, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Dropout(0.1),
            ...
            nn.AdaptiveAvgPool2d((1, 1)),  # → [N*S, slice_emb_dim, 1, 1]
            nn.Flatten(),                  # → [N*S, slice_emb_dim]
        )

        # 2) 1D conv‐based sequence processing (over 66 slices):
        self.conv1 = nn.Conv1d(
            in_channels=slice_emb_dim,
            out_channels=inter_dim,
            kernel_size=3,
        ...
        # 3) Global pooling over the 66 slices → [N, inter_dim]
        self.pool    = nn.AdaptiveAvgPool1d(output_size=1)
        self.flatten = nn.Flatten(start_dim=1)

        # 4) MLP to project from inter_dim → token_dim
        self.linear1 = nn.Linear(inter_dim, inter_dim)
        self.gelu    = nn.GELU()
        self.drop3   = nn.Dropout(0.1)
        self.linear2 = nn.Linear(inter_dim, token_dim)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        returns: [N, token_dim]
        """
        N, S, C, H, W = x.shape  # S = 66

        # → [N*S, 256, 32, 32]
        x = x.view(N * S, C, H, W)

        # per‐slice 2D encode → [N*S, slice_emb_dim]
        x = self.slice2d(x)

        # → [N, S, slice_emb_dim]
        x = x.view(N, S, self.slice_emb_dim)

        # → [N, slice_emb_dim, S]
        x = x.permute(0, 2, 1)

        # 1D conv block #1: [N, slice_emb_dim, S] → [N, inter_dim, S]
        x = self.conv1(x)
        x = self.relu1(x)
        x = self.drop1(x)
        x = self.bn1(x)

        # 1D conv block #2: [N, inter_dim, S] → [N, inter_dim, S]
        x = self.conv2(x)
        x = self.relu2(x)
        x = self.drop2(x)
        x = self.bn2(x)

        # global‐pool over the 66 slices → [N, inter_dim, 1]
        x = self.pool(x)

        # flatten → [N, inter_dim]
        x = self.flatten(x)

        # MLP projection: [N, inter_dim] → [N, inter_dim]
        x = self.linear1(x)
        x = self.gelu(x)
        x = self.drop3(x)
...
class MADPENetNoBackbonesSurv(nn.Module):  # ModalityAwareDPENet da decidere nome
    ...
    def __init__(
        ...
        self.rad_adapter = RadAdapter2p5D(
            slice_in_channels=256,
            slice_emb_dim=64,
        ...
        self.token_adapt_rad = TokenAdaptRad2p5D(
            slice_in_channels=256,
            slice_emb_dim=64,
            inter_dim=self.inter_dim,
    ...
    def forward(
        self,
        rad_feature,      # now expects a tensor of shape [B, 66, 256, 32, 32]
        histo_feature,    # unchanged, still [B, 768]
        modality_flag=None,
        output_layers=["hazard"]
    ):
        """
        Forward pass:
        rad_feature   (batch, slices, depth, height, width) = [B, 66, 256, 32, 32]
        histo_feature (batch, feature)                = [B, 768]
        """
        outputs = OrderedDict()

        # Build boolean masks for which samples have each modality
        ...
        ###
        # 1) Adapt radiology features (only where rad_mask is True)
        ###
        # Before: rad_adapter expected [N, 1024, 66], so we permuted. Now:
        # rad_adapter is a Conv3d‐based module that expects input [N, 66, 256, 32, 32].
        # So we drop the permute and call it directly on rad_feature[rad_mask].
        #
        # adapted_rad will have shape [N, inter_dim], where
        # N = rad_mask.sum().
        adapted_rad = self.rad_adapter(
            rad_feature[rad_mask]
        )  # shape = (num_rads, inter_dim)

        ###
        # 2) Adapt histology features (exactly as before)
        ###
        adapted_histo = self.histo_adapter(
            histo_feature[histo_mask]
        )  # shape = (num_histos, inter_dim)

        ###
        # 3) Scatter back into a full-[B, inter_dim] tensor, injecting missing‐tokens
        ###
        # Prepare empty tensors for full batch
        f_adapted_rad = torch.empty(batch_size, adapted_rad.shape[1], device=self.missing_rad_token.device)
        f_adapted_histo = torch.empty(batch_size, adapted_histo.shape[1], device=self.missing_rad_token.device)

        # Fill in where we have actual data
        f_adapted_rad[rad_mask]   = adapted_rad
        f_adapted_histo[histo_mask] = adapted_histo

        # For samples without a modality, repeat the learned “missing” token
        ...
        # If the user specifically asked for just the adapted_features output:
        ...
        ###
        # 4) Positional Embedding (unchanged)
        ###
        pe = self.dpe(
        ...
        ###
        # 5) Token‐adaptation + missing‐token injection for fusion
        ###
        # For radiology: the old code did rad_feature[rad_mask].permute(0,2,1) → [N, 1024, 66].
        # Now token_adapt_rad is Conv3d‐based (in_channels=66), so it expects [N, 66, 256, 32, 32].
        rad_tokens_pre = self.token_adapt_rad(
            rad_feature[rad_mask]
        )  # → (num_rads, token_dim)
        histo_tokens_pre = self.token_adapt_histo(
            histo_feature[histo_mask]
        )  # → (num_histos, token_dim)

        # Allocate space for full batch
        rad_tokens   = torch.empty(batch_size, rad_tokens_pre.shape[1],   device=self.missing_rad_token.device)
        histo_tokens = torch.empty(batch_size, histo_tokens_pre.shape[1], device=self.missing_histo_token.device)

        # Fill in the available tokens
        rad_tokens[rad_mask]     = rad_tokens_pre
        histo_tokens[histo_mask] = histo_tokens_pre

        # For missing modalities, inject the learned fusion‐missing tokens
        ...
        ###
        # 6) Attention‐based fusion (unchanged)
        ###
        f_att = self.fusion(
            rad_tokens,
            histo_tokens,
        ...
        ###
        # 7) Final hazard network (unchanged)
        ###
