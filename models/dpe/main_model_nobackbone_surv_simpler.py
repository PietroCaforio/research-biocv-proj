...
# TODO: Adatta il modello al caso survival

class DynamicPositionalEmbedding(nn.Module):
    def __init__(self, in_channels, out_channels, topk_pos= 256, topk_neg= 256):
        super().__init__()
...
class MADPENetNoBackbonesSurv(nn.Module):  # ModalityAwareDPENet da decidere nome
    ...
    def __init__(
        ...
        self.fusion = fusion_layer(d_model=self.token_dim,dim_hider=self.dim_hider,nhead=1, dropout=0.1)
        
        self.hazard_net = nn.Sequential(
            nn.Linear(self.token_dim, 1),  # First hidden layer
        )
        ...
        self.output_range = nn.Parameter(torch.FloatTensor([-3]), requires_grad = False)        
