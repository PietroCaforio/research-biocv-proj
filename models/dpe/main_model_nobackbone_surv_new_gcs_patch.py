...
class MADPENetNoBackbonesSurv(nn.Module):
    def __init__(
        ...
        #
        # 1) RAD_ADAPTER:   [B, 66, rad_input_dim] → [B, 66, inter_dim]
        #
        # We use two Conv1d layers (kernel_size=1) over the “channel=rad_input_dim” dimension,
        # keeping sequence_length=66 intact.  After each conv, we still have shape [B, inter_dim, 66].
        # Finally we permute back to [B, 66, inter_dim].
        #
        ...
        #
        # 2) HISTO_ADAPTER: [B, 66, histo_input_dim] → [B, 66, inter_dim]
        #
        ...
        # Missing‐modality tokens (now each must be repeated 66 times later)
        ...
        #
        # 3) TOKEN_ADAPT_RAD (unchanged): still takes raw [b1, 66, rad_input_dim] →
        #    conv → [b1, token_dim] (via average‐pooling over the 66 slots at the end).
        #
        ...
        #
        # 4) TOKEN_ADAPT_HISTO: [B, 66, histo_input_dim] → [B, token_dim]
        #
        #   (a) First project each of the 66 slots to inter_dim via same blocks as HistoAdapter
        #   (b) Then Conv1d→ token_dim over channel dimension, keep sequence length=66
        #   (c) AdaptiveAvgPool over the 66 “slots” to get a single [B, token_dim].
        #
        self.token_adapt_histo = nn.Sequential(
            ...
            # Project inter_dim → token_dim, still keeping length=66
            nn.Conv1d(
                in_channels=self.inter_dim,       # 512
        ...
        #    Each now maps [B, 66, inter_dim] → [B, 66, token_dim], i.e. applies per‐slot Linear.
        #
        self.token_adapt_rad_pe = nn.Sequential(
            nn.Linear(self.inter_dim, self.token_dim),  # [B, 66, 512] → [B, 66, 64]
            nn.GELU(),
            nn.Dropout(0.1),
            nn.Linear(self.token_dim, self.token_dim),  # [B, 66, 64] → [B, 66, 64]
            nn.LayerNorm(self.token_dim, eps=1e-5),
        )
    ...
    def forward(
        ...
        """
        rad_feature:   [B, 66, rad_input_dim]
        histo_feature: [B, 66, histo_input_dim]
        modality_flag: [B, 2] boolean mask

        We will produce:
          f_adapted_rad   → [B, 66, inter_dim]
          f_adapted_histo → [B, 66, inter_dim]
        """
        ...
        # ----------------------------
        # rad_feature[rad_mask] has shape [b1, 66, rad_input_dim]
        # Permute to [b1, rad_input_dim, 66] for Conv1d
        if rad_mask.any():
            _rad_seq = rad_feature[rad_mask].permute(0, 2, 1)  # [b1, 1024, 66]
        ...
        # ----------------------------
        # histo_feature[histo_mask] has shape [b2, 66, histo_input_dim]
        if histo_mask.any():
